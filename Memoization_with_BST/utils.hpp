...
int det_with_BST(Matrix mat_obj, BST* bst){
    BST_Node* root = bst->get_root();
    if(bst->key_exists(root, mat_obj) == 1){
        return bst->search(root, mat_obj);
    }
        ...
        // all other cases
            ...
            for(int i = 0; i < size; i++){
                ...
                long sub_det = det_with_BST(sub_matrix, bst);
                // insert the related matrix and its determinant to the Hash Table
                bst->insert(root, sub_matrix, sub_det);
